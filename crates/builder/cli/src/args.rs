...
mod tests {
    ...
    #[test]
    fn obsolete_resource_limit_flags_are_rejected() {
        for flag in [
            ...
            let error = CommandParser::try_parse_from(["builder", flag, "1"])
                .expect_err("obsolete resource limit flags must be rejected");

            assert!(error.to_string().contains("unexpected argument"), "{flag}: {error}");
        }
    }
