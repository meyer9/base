...
struct OpenLoopPresignConfig {
    ...
    base_fee: u128,
    max_gas_price: u128,
    prefill_per_sender: Option<usize>,
...
struct OpenLoopEnqueueProgress {
    ...
    presigned_remaining: Option<u64>,
...
impl OpenLoopHeadroomTarget {
    fn new(max_target_in_flight: u64, initial_confirmed_count: u64, sampled_at: Instant) -> Self {
    ...
    fn maybe_update(
        ...
        let updated_target_in_flight =
            target_with_batch_buffer.clamp(self.min_target_in_flight, self.max_target_in_flight);
...
impl LoadRunner {
    ...
    pub fn new(config: LoadConfig) -> Result<Self> {
        ...
        let recipient_keys = if config.fresh_recipient_ratio > 0.0 {
            ...
            let randomize_recipient_stream = config.fresh_recipient_ratio >= 1.0;
            let stream = if let Some(mnemonic) = &config.mnemonic {
                if randomize_recipient_stream {
                    let random_offset_delta = usize::try_from(random::<u32>()).map_err(|e| {
                        BaselineError::Config(format!(
                            "failed to convert randomized recipient offset delta to usize: {e}"
                        ))
                    })?;
                    let randomized_offset = offset.checked_add(random_offset_delta).ok_or_else(|| {
                        BaselineError::Config(format!(
                            "recipient_offset {offset} + randomized delta {random_offset_delta} overflows usize"
                        ))
                    })?;
                    ...
                    info!(
                        ...
                        recipient_random_offset_delta = random_offset_delta,
                    ...
                    info!(
                        fresh_recipient_ratio = config.fresh_recipient_ratio,
                        recipient_offset = offset,
                        "fresh-recipient mode enabled; recover addresses with AccountPool::from_mnemonic(mnemonic, n, recipient_offset)",
                    );
                    stream
                }
            } else {
                let recipient_seed =
                    if randomize_recipient_stream { random::<u64>() } else { config.seed };
                let stream = KeyStream::from_seed(recipient_seed, offset)?;
                if randomize_recipient_stream {
                    info!(
                        configured_seed = config.seed,
                        randomized_recipient_seed = recipient_seed,
                        fresh_recipient_ratio = config.fresh_recipient_ratio,
                        recipient_offset = offset,
                    ...
                    info!(
                        ...
                        fresh_recipient_ratio = config.fresh_recipient_ratio,
                        recipient_offset = offset,
                        "fresh-recipient mode enabled; recover addresses with AccountPool::with_offset(seed, n, recipient_offset)",
                    );
                }
                stream
            };
    ...
    fn estimate_avg_calldata_bytes(&self) -> u64 {
        ...
        for tx_config in &self.config.transactions {
            let bytes = match &tx_config.tx_type {
                ...
                | TxType::B20
                | TxType::B20Evm { .. }
                | TxType::BatchSettlementSettle => 68,
    ...
    pub async fn run(&mut self) -> Result<MetricsSummary> {
        ...
        let mut open_loop_progress: Option<OpenLoopEnqueueProgress> = None;
        ...
        if self.config.open_loop {
            ...
            let bounded_prefill_per_sender = (self.config.prefill_per_sender != 0)
                .then_some(self.config.prefill_per_sender as usize);
            let presigned_remaining = bounded_prefill_per_sender.map(|prefill_per_sender| {
                (prefill_per_sender as u64).saturating_mul(sender_count as u64)
            });

            let replacement_generator = Self::create_generator(
            ...
            let producer_task = tokio::spawn(Self::stream_open_loop_presigned_transactions(
                ...
                OpenLoopPresignConfig {
                    ...
                    max_gas_price: self.config.max_gas_price,
                    prefill_per_sender: bounded_prefill_per_sender,
                    fresh_recipient_ratio: self.config.fresh_recipient_ratio,
                    signed_chunk_tx,
                },
            ));

            let max_target_in_flight =
            ...
            if let Some(presigned_total) = progress.presigned_remaining {
                info!(
                    sender_count,
                    prefill_per_sender = self.config.prefill_per_sender,
                    presigned_total,
                ...
                info!(
                    ...
                    target_margin_multiplier = OPEN_LOOP_TARGET_MARGIN_MULTIPLIER,
                    target_lookahead_seconds = OPEN_LOOP_TARGET_LOOKAHEAD_SECONDS,
                    "started open-loop streaming pre-sign pipeline in unbounded mode"
                );
            }
            ...
            if let Err(err) = enqueue_result {
                warn!(
                    ...
                    presigned_remaining = progress.presigned_remaining.unwrap_or(0),
                    presign_unbounded = progress.presigned_remaining.is_none(),
            ...
            open_loop_progress = Some(progress);
        }
            ...
            && open_loop_enqueue_error.is_none()
        {
            ...
            if !self.config.open_loop
                && last_base_fee_refresh.elapsed() >= BASE_FEE_REFRESH_INTERVAL
            {
            ...
            if use_live_display || use_snapshot_tx {
                ...
                info!(
                    ...
                    total_queued,
                    senders_blocked,
                    presigned_generated = open_loop_progress
                        .as_ref()
                        .map_or(0, |progress| progress.presigned_generated),
                    presigned_remaining = open_loop_progress
                        .as_ref()
                        .and_then(|progress| progress.presigned_remaining)
                        .unwrap_or(0),
                    presign_unbounded = open_loop_progress
                        .as_ref()
                        .is_some_and(|progress| progress.presigned_remaining.is_none()),
                    base_fee = self.base_fee,
    ...
    async fn stream_open_loop_presigned_transactions(
        ...
        let mut signed_per_sender = 0usize;
        let mut chunk_index = 0usize;

        // `prefill_per_sender == None` means unbounded (stream forever); the
        // producer only stops when the consumer drops `signed_chunk_rx`
        // (detected below via the `send(...).is_err()` check), which happens
        // once the enqueue loop returns (deadline reached or channel closed).
        while config.prefill_per_sender.is_none_or(|cap| signed_per_sender < cap) {
            let txs_per_sender = config
                .prefill_per_sender
                .map_or(chunk_per_sender, |cap| (cap - signed_per_sender).min(chunk_per_sender));
            ...
            signed_per_sender = signed_per_sender.saturating_add(txs_per_sender);
            let nonce_increment = u64::try_from(txs_per_sender).map_err(|e| {
                BaselineError::Transaction(format!(
            ...
            debug!(
                ...
                txs_per_sender,
                signed_per_sender,
                prefill_per_sender = ?config.prefill_per_sender,
                "open-loop pre-sign producer generated chunk"
            );
        }
    ...
    async fn enqueue_open_loop_signed_transactions(
                    ...
                    if let Some(signed_tx) = sender_iter.next() {
                        ...
                        if pending_signed_batch.len() >= OPEN_LOOP_SIGNED_BATCH_SIZE {
                            ...
                            if !Self::enqueue_open_loop_signed_batch(
                                ...
                                &mut progress.presigned_remaining,
                                drain_state,
                            )
                            .await
                            {
        ...
        if !pending_signed_batch.is_empty() {
            if let Some(update) = progress
                .headroom_target
                .maybe_update(Instant::now(), drain_state.collector.confirmed_count() as u64)
            {
                debug!(
                    previous_target_in_flight = update.previous_target_in_flight,
                    updated_target_in_flight = update.updated_target_in_flight,
    ...
    async fn enqueue_open_loop_signed_batch(
        ...
        presigned_remaining: &mut Option<u64>,
        drain_state: &mut OpenLoopDrainState<'_>,
    ) -> bool {
        ...
        match Self::enqueue_signed_while_draining(submission_pipeline, batch, drain_state).await {
            Ok(()) => {
                if let Some(remaining) = presigned_remaining.as_mut() {
