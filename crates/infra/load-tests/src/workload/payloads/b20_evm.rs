...
mod tests {
    ...
    fn generate_targets_fixed_contract() {
        ...
        let payload = B20EvmTransferPayload::new(contract, U256::from(1000), U256::from(1000));
        ...
        assert_eq!(tx.input.input().expect("input set").as_ref(), expected.abi_encode().as_slice(),);
    }
}
