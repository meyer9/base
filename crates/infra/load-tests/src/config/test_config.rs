...
pub struct TestConfig {
    ...
    /// the same derivation as the sender pool (mnemonic when set, otherwise `seed`). This forces
    /// account-trie fan-out for payloads that honor the runner-supplied `to` address.
    ///
    /// Recipient keys are advanced past the sender keys, so you can recover
    /// recipients out-of-band via
    /// `AccountPool::from_mnemonic(mnemonic, fresh_recipient_count, sender_offset + sender_count)`
    /// or `AccountPool::with_offset(seed, fresh_recipient_count, sender_offset + sender_count)`.
    /// The runner prints `recipient_offset` at startup and writes `fresh_recipient_count` to the
    /// final summary.
    #[serde(default)]
