...
- Use `simplifier`, `integration_refactorer`, `performance_engineer`, or `test_breaker` only for a substantive validated code improvement.
...
- Before opening a PR, confirm that the diff itself—not just its description—demonstrates the intended improvement and that validation exercises it. If no substantive, safely validated improvement is available in scope, make no repository change: do not add a review/report file, commit, push a branch, or open a PR. Move to another scoped opportunity; report a null result only directly to the requester when they asked for the investigation.
